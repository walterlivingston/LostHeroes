    ...
    protected void buildShapelessRecipes(Consumer<IFinishedRecipe> consumer) {
        for(Metal m : Metal.values()){
            if(m.isVanilla()){
                ...
                ShapedRecipeBuilder.shaped(LHItems.helmets.get(m)).define('X', LHItems.ingots.get(m)).pattern("X X").pattern("X X").unlockedBy("has_"+m.tagName(), has(LHItems.ingots.get(m))).save(consumer);
